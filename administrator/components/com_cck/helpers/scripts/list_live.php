...
if ( count( $fields ) ) {
	foreach ( $fields as $pos ) {
		foreach ( $pos as $field ) {
			...
			$variation	=	HTMLHelper::_( 'select.genericlist', $opts, 'variation_'.$field->name, 'class="inputbox variation_values" onchange="CCK_setOptions(\'variation\');"', 'value', 'text', $variat );
