...
<div class="<?php echo $this->css['filter']; ?>" id="filter-bar">
	<div class="<?php echo $this->css['filter_search']; ?>">
		...
		echo JCckDev::getForm( $cck['core_filter_input'], $this->escape( $this->state->get( 'filter.search' ) ), $config, array( 'attributes'=>'placeholder="'.Text::_( 'COM_CCK_ITEMS_SEARCH_FILTER' ).'" style="text-align:center;"' ), array( 'after'=>"\n" ) );
	...
	<div class="<?php echo $this->css['filter_search_list']; ?>">
		...
		<select name="directionTable" id="directionTable" class="inputbox select input-medium" onchange="Joomla.orderTable()">
			<option value="asc"<?php echo $selected_asc; ?>><?php echo Text::_( 'JGLOBAL_ORDER_ASCENDING' ); ?></option>
			<option value="desc"<?php echo $selected_desc; ?>><?php echo Text::_( 'JGLOBAL_ORDER_DESCENDING' );  ?></option>
		</select>
