	...
	JCck.Dev = {
			...
			$.ajax({
				...
				success: function(response){
					if (layout == "edit2") {
						...
						JCck.Dev.updateTrash("#sortable3");
						JCck.Dev.ajaxLayer("type", "edit4", "#sortable3", mydata, uix);
					} else if (layout == "edit4") {
						document.getElementById("sortable3").innerHTML = response;
						JCck.Dev.updateTrash("#sortable3");
					} else {
						$("#loading").html(""); $(elem).html(response); JCck.Dev.setEdit2(uix);
					}
				},
		...
		setEdit2: function(uix) {
			...
			$("#sortable1, #sortable2").selectable({
				filter		: "li.field",
				cancel		: "input,img,option,select,span.c_live,span.c_live2,span.c_mat,span.c_val,span.c_typo,span.c_link,span.c_res,span.c_comp,span.c_cond,.cbox,.ui-state-disabled,span.c_cancelled,.sp2se,.se2sp",
			});
			/* -- */
