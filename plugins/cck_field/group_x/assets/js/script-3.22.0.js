...
(function ($){
	JCck.More.GroupX = {
		add: function(name,max_element,new_elem,rid) {
			var ind_elem, length, position, Child, mode, container,
				...
				if (length < max_element) {
					...
					switch ( position ) {
						case 'first':
							...
							JCck.More.GroupX.insert(container,new_elem,ind_elem,name,mode);
		...
		insert: function(container,new_elem,ind_elem,name,mode) {
			if ( mode == 'after' ) {
				container.after( update_empty_group(new_elem,ind_elem,name) );
			} else {
				container.append( update_empty_group(new_elem,ind_elem,name) );
			}
		},
	...
	*/
	...
	function update_group_i( group_i, op_type, name, rid) {
		...
		buttons.attr('id',rid+'_button_'+name+'_'+new_ind_group);
		...
		forms.find('[id^="'+rid+'_'+name+'_'+num_group+'_"]').each(function( index ) {
			newattr = $(this).attr("id").replace( rid+'_'+name+"_"+num_group, rid+'_'+name+"_"+new_ind_group );
			$(this).attr("id", newattr );
		});

		group_i.find("script").each(function( index ) {
			var reg = new RegExp(name+'_'+num_group, "g");
			$(this).text( $(this).text().replace(reg,name+'_'+new_ind_group));
		});
	}
