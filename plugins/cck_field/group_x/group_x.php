...
class plgCCK_FieldGroup_X extends JCckPluginField
{
		...
		if ($count)
		{
			if ($field->bool == 2)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					$html .= self::renderTR($field, $field->form[$i], $i, $count, $rId, $js, $js2);
				}
				...
				$empty = self::renderTR($field, $field->form[$i], 0, 0, $rId, $js, $js2);;
			}
			else
			{
				$orientation = ($field->bool == 1)
					? 'vertical_gx'
					: 'horizontal_gx';
				$width = '';
	...
	private static function renderTR($field, $elements, $i, $count, $rId, &$js, &$js2)
	{
		$class = (($i % 2))
			? 'even'
			: 'odd';

		$formsClass = $formClass = '';

		if ($count == 1)
		{
			$formsClass = ' cck_form_group_x_first cck_form_group_x_last';
			$formClass  = ' cck_cgx_form_first cck_cgx_form_last';
		}
		elseif ($i == 0 && $count != 0)
		{
			$formsClass = ' cck_form_group_x_first';
			$formClass  = ' cck_cgx_form_first';
		}
		elseif ($i == $count - 1)
		{
			$formsClass = ' cck_form_group_x_last';
			$formClass  = ' cck_cgx_form_last';
		}

		$html = '<tr id="' . $rId . '_forms_' . $field->name . '_' . $i . '" class="' . $class . ' cck_form cck_form_group_x' . $formsClass . '">';
		foreach ($elements as $elem)
		{
			if ($elem->display > 1 && $elem->markup != 'none')
			{
				$html .= '<td id="' . $rId . '_form_' . $field->name . '_' . $i . '" class="cck_' . $elem->name . ' cck_cgx cck_cgx_form' . $formClass . '">';
			}
			$html .= $elem->form;
			...
			if (@$elem->computation)
			{
				...
				if ($computation_options->calc == 'custom')
				{
					...
					$event   = @$computation_options->event
						? $computation_options->event
						: 'keyup';
					$targets = @$computation_options->targets
						? json_encode($computation_options->targets)
						: '[]';
					$format  = '';
					...
					{
						$config['computation'][$event][] = array(
							'_'  => str_replace('#', '#' . $field->name . '_' . $i . '_', $elem->computation),
		...
		$html .= '<td class="">';
		$html .= self::_formHTML($field, array(), $i, $count, $config);
		$html .= '</td>';
		$html .= '</tr>';
	...
	protected static function _formHTML( $field, $group, $i, $size_group, &$config )
	{
		...
		$formsClass = $formClass = $buttonClass = '';

		if ($size_group == 1)
		{
			$formsClass  = ' cck_form_group_x_first cck_form_group_x_last';
			$formClass   = ' cck_cgx_form_first cck_cgx_form_last';
			$buttonClass = ' cck_cgx_button_first cck_cgx_button_last';
		}
		elseif ($i == 0 && $size_group != 0)
		{
			$formsClass  = ' cck_form_group_x_first';
			$formClass   = ' cck_cgx_form_first';
			$buttonClass = ' cck_cgx_button_first';
		}
		elseif ($i == $size_group - 1)
		{
			$formsClass  = ' cck_form_group_x_last';
			$formClass   = ' cck_cgx_form_last';
			$buttonClass = ' cck_cgx_button_last';
		}

		$html = '<div id="' . $rId . '_forms_' . $field->name . '_' . $i . '" class="cck_form cck_form_group_x' . $formsClass . '">';
		$html .= '<aside id="' . $rId . '_button_' . $field->name . '_' . $i . '" class="cck_cgx cck_cgx_button' . $buttonClass . '">' . $html_div_buttons . '</aside>';
		$html .= '<div id="' . $rId . '_form_' . $field->name . '_' . $i . '" class="cck_cgx cck_cgx_form' . $formClass . '">';
