...
class JCckPluginField extends JPlugin
{
	...
	public static function g_onCCK_FieldConstruct_TypeForm( &$field, $style, $data, $config = array() )
	{
		...
		if ( !$data['variation'] ) {
			...
			$text			=	$data['variation'][$value]->text;
	...
	public static function g_onCCK_FieldConstruct_SearchSearch( &$field, $style, $data, $config = array() )
	{
		...
		if ( !$data['match_mode'] ) {
			...
			$text				=	$data['match_mode'][$value]->text;
		...
		if ( $variation == 'value' ) {
			...
			if ( count( $options ) ) {
				...
				if ( !$loaded ) {
					...
					$js			=	'$("form#'.$parent.'").on("click", ".list-variation-items > li a", function() { var v = $(this).parent().attr("data-value"); $("#'.$id.'").val(v);'.$then.' });';
