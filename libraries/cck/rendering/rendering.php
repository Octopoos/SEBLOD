...
class CCK_Rendering {
    ...
    public function __call($method, $args) {
        ...
        if ($prefix == 'get') {
            ...
            if ($count == 1) {
                ...
                if ($count == 2) {
                    return empty($property) ? @$this->me[$fieldname]->{$this->mode_property}[$args[1]] : @$this->me[$fieldname]->{$this->mode_property}[$args[1]]->$property;
                } else {
                    return empty($property) ? @$this->me[$fieldname]->{$this->mode_property}[$args[1]]->{$this->mode_property}[$args[2]] : @$this->me[$fieldname]->{$this->mode_property}[$args[1]][$args[2]]->$property;
                }
            }
        }
    }
    ...
    public function setHeight($height, $id, $class = '', $markup = '') {
        if ($height) {
            $class = ( $class ) ? ' .' . $class : '';
            Factory::getApplication()->getDocument()->addStyleDeclaration('#' . $id . $class . ' ' . $markup . '{height:' . $height . ';}');
        }
    }
    ...
    public function addScript($url, $type = 'text/javascript', $defer = false, $async = false) {
        ...
        Factory::getApplication()->getDocument()->addScript($url, $options, $attribs);
    }
    ...
    public function addStyleSheet($url, $type = 'text/css', $media = null, $attribs = array()) {
        ...
        Factory::getApplication()->getDocument()->addStyleSheet($url, $options, $attribs);
    }
